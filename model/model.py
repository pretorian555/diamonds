...
def _get_hyperparameters() -> kerastuner.HyperParameters:
  ...
  hp.Choice('learning_rate', [1e-1,1e-2, 1e-3], default=1e-1)
  hp.Int('n_layers', 1, 2, default=1)
  with hp.conditional_scope('n_layers', 1):
        hp.Int('1_layer_n_units_1', min_value=8, max_value=128, step=8, default=8)
  with hp.conditional_scope('n_layers', 2):
        hp.Int('2_layer_n_units_1', min_value=8, max_value=128, step=8, default=8)
        hp.Int('2_layer_n_units_2', min_value=8, max_value=128, step=8, default=8)        
...
def _build_keras_model(hparams: kerastuner.HyperParameters, 
  ...
  for n in range(int(hparams.get('n_layers'))):
    layers = hparams.get('n_layers')
##    x = tf.keras.layers.Dense(units=hparams.get('n_units_' + str(n +1)), activation='relu')(concat_inputs)
    x = tf.keras.layers.Dense(units=hparams.get(f"{layers}_layer_n_units_" + str(n +1)), activation='relu')(concat_inputs)
...
#          'callbacks':[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3)]
      ...
      log_dir='tmp/tb', update_freq='batch')]
      })
